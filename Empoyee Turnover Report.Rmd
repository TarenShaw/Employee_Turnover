...
max_n_leaves_year <- df_count(n_leavers) %>% filter(n == max(n))
para <- if (max_n_leaves_year == max_year) {
F
} else {
T
}  
...
`r if(para){"xxx, as seen in fig x has the highest number of leavers so far"}` `r f(para){max_year}`
From fig x we can conlude that 
